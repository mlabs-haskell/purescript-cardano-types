...
toCsl = case _ of
  ...
  convertPlutusMap mp = plutusData_newMap $ packMapContainer $ map (\(Tuple k v) -> Tuple (toCsl k) (toCsl v)) mp
...
fromCsl pd = unsafePartial $ fromJust $
  ...
  convertPlutusMap pm =
    Map $ map (\(Tuple k v) -> Tuple (fromCsl k) (fromCsl v)) $ Array.zip keys values
    where
    keys = unpackListContainer $ plutusMap_keys pm
    values = unpackListContainer $ plutusMap_values pm
