...
instance EncodeAeson EnterpriseAddress where
  encodeAeson = toCsl >>> enterpriseAddress_toAddress >>> flip address_toBech32 (unsafeCoerce undefined) >>> encodeAeson

instance DecodeAeson EnterpriseAddress where
  decodeAeson = note (TypeMismatch "EnterpriseAddress") <<< decodeBech32 <=< decodeAeson
    where
    decodeBech32 = map fromCsl <<< toMaybe <<< enterpriseAddress_fromAddress <=< fromBech32
...
toCsl = case _ of
  EnterpriseAddress nid sc ->
    enterpriseAddress_new (Int.toNumber $ NetworkId.toInt nid) (Credential.toCsl $ unwrap sc)

fromCsl :: Csl.EnterpriseAddress -> EnterpriseAddress
fromCsl addr =
  EnterpriseAddress networkId (wrap $ Credential.fromCsl $ enterpriseAddress_paymentCred addr)
  where
  networkId :: NetworkId
