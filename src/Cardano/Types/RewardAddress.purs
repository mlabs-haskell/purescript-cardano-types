...
instance DecodeAeson RewardAddress where
  decodeAeson = note (TypeMismatch "RewardAddress") <<< fromBech32 <=< decodeAeson

toBech32 :: RewardAddress -> Bech32String
toBech32 = toCsl >>> rewardAddress_toAddress >>> flip address_toBech32 (unsafeCoerce undefined)
...
toCsl = case _ of
  RewardAddress nid sc ->
    rewardAddress_new (Int.toNumber $ NetworkId.toInt nid) (Credential.toCsl $ unwrap sc)

fromCsl :: Csl.RewardAddress -> RewardAddress
fromCsl addr =
  RewardAddress networkId (wrap $ Credential.fromCsl $ rewardAddress_paymentCred addr)
  where
  networkId :: NetworkId
